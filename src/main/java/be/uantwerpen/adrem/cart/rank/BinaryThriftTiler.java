...
public class BinaryThriftTiler
{
	private double[][] dims;
	...
	public BinaryThriftTiler(InputFile inputFile)
	{
		try
		{
	...
	public List<Tile> runFor(int dimIx, int k, int topK)
	{
		...
		for (int tileIx = 0; tileIx < topK; tileIx++)
		{
			...
			{
				int headLength = tile.sr - items[c].txS;
				int tailLength = items[c].txE - tile.er;
				if (headLength <= tailLength)
				{
					items[c].txS = tile.er + 1;
				} else
				{
					items[c].txE = tile.sr - 1;
				}
			}
