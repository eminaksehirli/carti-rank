...
public class BinarySquareTiler
{
	private double[][] dims;
	...
	public BinarySquareTiler(InputFile inputFile)
	{
		try
		{
	...
	public List<Tile> runFor(int dimIx, int k, int topK)
	{
		...
		for (int tileIx = 0; tileIx < topK; tileIx++)
		{
			...
			Tile fixedTile;
			if (ec - sc < tile.er - tile.sr)
			{
				fixedTile = new Tile(tile.score, sc, sc, ec, ec);
			} else
			{
				fixedTile = new Tile(tile.score, tile.sr, tile.sr, tile.er, tile.er);
			}
	...
	protected Tile findMaxTile(Item[] items)
	{
		...
		for (int start = 0; start < items.length - 1; start++)
		{
			...
			for (; end < items.length && items[end].txS <= items[start].txE; end++)
			{
				int eStart = start;
				int eEnd = end;
				if (end - start > items[start].txE - items[end].txS)
				{
					eStart = items[end].txS;
					eEnd = items[start].txE;
				}
				int edgeSize = eEnd - eStart;
				int score = edgeSize * edgeSize;
				if (score > maxScore)
				{
					maxScore = score;
					maxTile[0] = start;
